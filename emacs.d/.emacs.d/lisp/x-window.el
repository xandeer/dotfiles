...
;;; dimmer
(x/package-use 'dimmer)

(setq dimmer-fraction 0.38
      dimmer-adjustment-mode :foreground
      dimmer-use-colorspace :rgb)

(with-eval-after-load 'which-key
  (when (fboundp 'dimmer-configure-which-key)
    (dimmer-configure-which-key)))

(x/append-init-hook (lambda () (dimmer-mode 1)))
...
;;; golden ratio
(defun x/golden-ratio-inhibit-p ()
  "Return non-nil when the current window should skip `golden-ratio'."
  (or (string-prefix-p "telega-" (symbol-name major-mode))
...
(with-eval-after-load 'golden-ratio
  (dolist (command '(ace-window x/flip-window))
    (add-to-list 'golden-ratio-extra-commands command))
  (add-to-list 'golden-ratio-inhibit-functions
               #'x/golden-ratio-inhibit-p))

(setq golden-ratio-auto-scale t)
(setq golden-ratio-max-width 110)
(golden-ratio-mode 1)
