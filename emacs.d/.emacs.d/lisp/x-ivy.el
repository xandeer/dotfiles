;;; x-ivy.el --- ivy -*- lexical-binding: t; -*-
;;; Commentary:
;;; Code:
...
(leaf counsel
  ...
  (defun x/counsel-buffer-or-recentf-other-window ()
    ...
    (ivy-read "Switch to buffer or recentf in another window:"
              ...
              :caller 'x/counsel-buffer-or-recentf-other-window))
  ...
  (ivy-add-actions
   ...
   '(("O" x/ivy-git-grep-other-window-action "open in other window")))
...
(leaf ivy-prescient
  ...
  :config
  (defun x/enable-pinyin ()
    (interactive)
    (setq prescient-filter-method '(literal regexp initialism pinyin)))
  (defun x/disable-pinyin ()
    (interactive)
    (setq prescient-filter-method '(literal regexp initialism)))
  (x/disable-pinyin)
  (add-hook 'minibuffer-exit-hook 'x/disable-pinyin)
  (add-hook 'minibuffer-setup-hook 'x/enable-pinyin))

(leaf all-the-icons-ivy-rich
  :straight t
  :defvar x/all-the-icons-ivy-rich-reload-p
  :custom
  (all-the-icons-ivy-rich-icon-size . 0.7)
  :init
  (all-the-icons-ivy-rich-mode 1)
  (setq x/all-the-icons-ivy-rich-reload-p nil)
  (defun x/ivy-rich-reload ()
    (if (and all-the-icons-ivy-rich-mode
             x/all-the-icons-ivy-rich-reload-p)
        (advice-remove #'counsel-M-x #'x/ivy-rich-reload)
      (all-the-icons-ivy-rich-reload)
      (setq x/all-the-icons-ivy-rich-reload-p t)))
  (defun x/all-the-icons-ivy-rich-align-icons ()
    "Set tab size to 1, to insert tabs as delimiters."
    (setq-local tab-width 2))
  :advice
  (:before counsel-M-x x/ivy-rich-reload)
  (:override all-the-icons-ivy-rich-align-icons x/all-the-icons-ivy-rich-align-icons))
...
;;;###autoload
(defun x/region-active-p ()
  "Return non-nil if selection is active."
  (declare (side-effect-free t))
  (use-region-p))

;;;###autoload
(defun x/region-beginning ()
...
;;;###autoload
(defun x/thing-at-point-or-region (&optional thing prompt)
...
;;;###autoload
(cl-defun x/ivy-file-search (&key query in all-files (recursive t) prompt args)
  ...
  (require 'counsel)
  (defun x/project-root (&optional dir)
...
Returns nil if not in a project."
  ...
  (let* ((this-command 'counsel-rg)
         (project-root (or (x/project-root) default-directory))
            ...
            "[! |]" (lambda (substr)
                      (cond ((and (string= substr " ")
                            ...
                            ((string= substr "|")
                             "\\\\\\\\|")
                            ((concat "\\\\" substr))))
            (rxt-quote-pcre (x/thing-at-point-or-region)))))
     directory args
     (or prompt
         (format "rg%s [%s]: "
                 args
...
;;;###autoload
(defun x/ivy/project-search (&optional arg initial-query directory)
  "Performs a live project search from the project root using ripgrep.
  If ARG (universal argument), include all files, even hidden or compressed ones,
  in the search."
  (interactive "P")
  (x/ivy-file-search :query initial-query :in directory :all-files arg))

;;;###autoload
(defun x/ivy/project-search-from-cwd (&optional arg initial-query)
  "Performs a project search recursively from the current directory.
  If ARG (universal argument), include all files, even hidden or compressed ones."
  (interactive "P")
  (x/ivy/project-search arg initial-query default-directory))

;;;###autoload
(defun x/search-cwd (&optional arg)
...
If prefix ARG is set, prompt for a directory to search from."
  ...
  (let ((default-directory
          (if arg (counsel-read-directory-name "Search directory: ")
            default-directory)))
    (call-interactively #'x/ivy/project-search-from-cwd)))

;;;###autoload
(defun x/search-other-cwd ()
