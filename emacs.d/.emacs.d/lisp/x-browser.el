...
(defun x/browse-url-with-external-browser (target)
  "Browse TARGET with the configured external browser.

TARGET may be a URL string or a bookmark record containing a `url' entry."
  (let ((url (cond
              ((stringp target) target)
              ((and (consp target)
                    (stringp (car target)))
               (alist-get 'url (cdr target)))
              ((consp target)
               (alist-get 'url target)))))
    (unless (stringp url)
      (error "Unsupported browser target: %S" target))
    (require 'browse-url)
  ...
  (setq x/buku-bookmarks
        (mapcar (lambda (bm)
                  `(,(alist-get 'title bm)
                    ...
                    (handler . x/browse-url-with-external-browser)))
                ebuku-bookmarks))
...
(defun x/open-bookmark (title)
  (if (assoc title bookmark-alist)
    ...
    (let ((url (alist-get 'url (assoc title x/bookmarks))))
      (x/browse-url-with-external-browser url))))
