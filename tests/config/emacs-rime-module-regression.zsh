...
(progn
  ...
  (unwind-protect
      (progn
        ...
        (dolist (key (string-to-list \"harness\"))
          (unless (rime-lib-process-key key 0)
            (error \"second Return input was not accepted: %c\" key)))
        (unless (rime-lib-process-key 65293 0)
          (error \"second Return was not accepted\"))
        (unless (equal (rime-lib-get-commit) \" harness\")
          (error \"second Han-to-Return spacing was not committed\"))
        (when (rime-lib-process-key 32 0)
          (error \"idle manual space was unexpectedly handled\"))
        (when (rime-lib-get-commit)
          (error \"idle manual space unexpectedly produced a commit\"))
        (unless (rime-lib-process-key ?a 0)
          (error \"post-manual-space input was not accepted\"))
        (let* ((context (rime-lib-get-context))
               (menu (alist-get 'menu context))
               (first (caar (alist-get 'candidates menu))))
          (unless (equal first \"测试\")
            (error \"manual space did not fence auto spacing: %S\" first)))
        (unless (rime-lib-process-key 32 0)
          (error \"unspaced AI candidate selection was not accepted\"))
        (unless (equal (rime-lib-get-commit) \"测试\")
          (error \"manual-space-to-Han commit was unexpectedly prefixed\")))
...
(progn
  ...
  (setq user-emacs-directory \"$tmp/emacs/\")
  (load-file \"$x_rime\")
  (unless (memq #'x/rime-predicate-after-alphabet-char-p
                rime-disable-predicates)
    (error \"space-aware alphabet predicate is not installed\"))
  (when (memq #'rime-predicate-after-alphabet-char-p
              rime-disable-predicates)
    (error \"direct alphabet predicate still bypasses manual space\"))
  (with-temp-buffer
    (insert \"eng\")
    (let ((rime-active-mode nil)
          (rime--temporarily-ignore-predicates nil)
          (rime-disable-predicates
           '(x/rime-predicate-after-alphabet-char-p))
          processed)
      (cl-letf (((symbol-function 'rime--rime-lib-module-ready-p)
                 (lambda () t))
                ((symbol-function 'rime-lib-get-context)
                 (lambda () '((composition (length . 0)))))
                ((symbol-function 'rime-lib-process-key)
                 (lambda (key mask)
                   (setq processed (cons key mask))
                   nil))
                ((symbol-function 'rime-lib-get-commit)
                 (lambda () nil))
                ((symbol-function 'rime--should-inline-ascii-p)
                 (lambda () nil))
                ((symbol-function 'rime--refresh-mode-state)
                 (lambda () nil)))
        (unless (equal (rime-input-method 32) '(32))
          (error \"manual space was not returned to Emacs\"))
        (unless (equal processed '(32 . 0))
          (error \"manual space bypassed librime: %S\" processed))
        (setq processed nil)
        (unless (equal (rime-input-method ?x) '(?x))
          (error \"alphabet passthrough changed\"))
        (when processed
          (error \"alphabet passthrough reached librime: %S\" processed)))))
