...
current_snapshot = method(source, "currentAISnapshot")
...
reject_match(source, /\buniqueClientIdentifierString\s*\(/, "AI bridge must not call an unstable IMK client identifier API")
reject_match(current_snapshot, /\bclientID\s*:/, "currentAISnapshot must not store an unstable IMK client identifier")
require_match(current_snapshot, /session:\s*UInt64\(session\)/, "snapshot must retain Rime session identity")
require_match(current_snapshot, /generation:\s*generation/, "snapshot must retain request generation identity")
require_match(current_snapshot, /appID:\s*currentApp/, "snapshot must retain application identity")
